...
public class ScoringMatrix extends ScoringScheme
{
		...
		throws IncompatibleScoringSchemeException
	{
        ...
        // Get index of a char for faster alignments and to be used in scoreSubstitution(int a, int b)
        public int getIndexOfChar(char a)
        {
		if (a == '-') 
			a = '*';
        ...
        public int[] getIndexesOfSequences(String seq)
        {
            int[] sequenceIndex = new int[seq.length()];
            
            for (int i=0; i<sequenceIndex.length; i++)
            {
                sequenceIndex[i] = getIndexOfChar(seq.charAt(i));
            }
            return sequenceIndex;
        }
        
        // Faster version for faster alignments
        public int scoreSubstitution (int r, int c)
        {
            if (r < 0 || c < 0)
                    return -5;            
            return matrix[r][c];
